function Single_point_slopeangle_450(Project, basedir, Phases, tag, varargin)
...
%         ' phase Velocity_Model s_inds slons slats folder station_at_cp time_difference name project Sca_info']) %depths  RP
    ...
    save([basedir '/Data/CCP/AllNtwrks_' phase '_' name],'RF_depth','CP_lat','CP_lon','RP','baz','SLAT','SLON','cp_depths','depth_indx',...
...
%     clear Sca_info
 ...
 %   [latTopo,lonTopo,Topo] = satbath(1,LatLims,LonLims_Temp);
  ...
  %  cd(basedir)
    ...
    %     parfor kkk=1:rfi
    for kkk=1
        parcalculate(kkk,parnum,basedir,FileNames,RF_depth,tag,s_inds,RP,baz,Inter_Velo,model_lats,model_lons,CP_lat,CP_lon,cp_depths,time_difference,depth_indx,Phase,phase,tmplat,tmplon,latTopo,lonTopo,Topo,SLON,SLAT)
...
function parcalculate(kkk,parnum,basedir,FileNames,RF_depth,tag,s_inds,RP,baz,Inter_Velo,model_lats,model_lons,CP_lat,CP_lon,cp_depths,time_difference,depth_indx,Phase,phase,tmplat,tmplon,latTopo,lonTopo,Topo,SLON,SLAT)
...
lat_tar = 40.4699;
lon_tar = 38.0030;
ind_tar = find(abs(model_lats-lat_tar)<0.01 & abs(model_lons-lon_tar)<0.01);

%num = 1;
savename=[basedir 'Data/CCP/Single_Point_lat405_lon38.mat'];
...
for i = 1:length(Inter_Velo)
    ...
    if mod(i,30)==1 && i~=1
        parsave(savename,'phase',phase,'plane_RF',plane_RF,'RP_means',RP_means,'tmplat',tmplat,'tmplon',tmplon,'cp_depths',cp_depths,...
            ...
            'model_lons',model_lons,'n_events',n_events,'weighted_n_events',weighted_n_events,'cur_sta',i,'clean_indx',clean_indx,'num_indx',num_indx,'all_weight',all_weight,'all_rf',all_rf);
    end
...
%         model_lonsec = repmat(permute(model_lons(ind_c),[2,1,3]),[length(depth_indx),1,length(sec_ind)]);
        ...
        all_weight = cat(2,all_weight,permute(Weightsec(:,ind_tar,:),[1,3,2]));
        all_rf = cat(2,all_rf,(RF_depth(sec_ind,:))');
...
parsave(savename,'phase',phase,'plane_RF',plane_RF,'RP_means',RP_means,'tmplat',tmplat,'tmplon',tmplon,'cp_depths',cp_depths,'weighting_norm',weighting_norm,...
    'latTopo',latTopo,'lonTopo',lonTopo,'Topo',Topo,'SLON',SLON,'SLAT',SLAT,'model_lats',model_lats,'w2sum',w2sum,'w2RF2sum',w2RF2sum,'w2RFsum',w2RFsum,...
    'model_lons',model_lons,'n_events',n_events,'weighted_n_events',weighted_n_events,'wRFsum',wRFsum,'clean_indx',clean_indx,'num_indx',num_indx,'std_RF',std_RF,'all_weight',all_weight,'all_rf',all_rf);
end
