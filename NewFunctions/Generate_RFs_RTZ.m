function Generate_RFs_RTZ(Project, basedir, NETs, Phases, tag, varargin)
            ...
            if strcmp(iffiltdat,'yes')
                ...
                for in = 1:length(NETs)
                    ...
                    for is=1:length(STAs)
                        ...
                        load([Proj_Dir NETs{in} '/' STAs{is} '/Ray_Path_Data_',num2str(round(1/Params.hf)),'_',num2str(round(1/Params.lf)),'.mat']);
                        load([Proj_Dir NETs{in} '/' STAs{is} '/Migration_Models_',num2str(round(1/Params.hf)),'_',num2str(round(1/Params.lf)),'.mat']);
        ...
        for iph=1:length(Phases)
            ...
            for in=1:length(NETs)
                ...
                for iu = 1:length(Unis)
                    ...
                    if strcmp(ifplot,'yes') && size(rfs_sta,1)>40
                    ...
                    save([Proj_Dir '/SingleRF/' NETs{in} '_' STAref '_' Phase '_' num2str(bazbins(mind)) '_',num2str(lowT),'_',...
                        num2str(highT),'.mat'],'depth','bootstrap','nrfs');
...
if(length(Project)>6 && strcmp(Project(1:6),'Synth_'))
    ...
    if strcmp(tagdeconv,'HK')
        ...
        savename=['_' Params.Phase '_' num2str(round(1/Params.hf)) '_' ...
            num2str(round(1/Params.lf)) 's_' datestr(date,'dd.mm.yy') '.mat'];
    end
    tag='data';
end
    ...
    for is=1:length(STAs)
        ...
        if ~strcmp(tagdeconv,'HK')
            ...
            if strcmp(tag,'synth')
                ...
                MG_File = [Data_Dir 'Migration_Models_',num2str(round(1/Params.hf)),'_',num2str(round(1/Params.lf)),'.mat'];
                tag='data';
            end
        ...
        if ~isempty(Event_IDs)
            ...
            if ~isempty(Waveform_Data)
                ...
                Deconvolve_Data(Waveform_Data,Bin_Data,Params,...
                    dt,NET,STA,Phase,Data_Dir,round(1/Params.hf),round(1/Params.lf), tagdeconv,savename);
...
if ~isempty(Event_IDs)
    ...
    for ie=1:length(Event_IDs)
        ...
        if(strcmpi(Params.PreFilter,'no'))
            ...
            R   = All_WV_Data.(eq_id).([Phase '_Waveforms']).R;
            T   = All_WV_Data.(eq_id).([Phase '_Waveforms']).T;
            Z   = All_WV_Data.(eq_id).([Phase '_Waveforms']).Z;
        elseif strcmpi(Params.PreFilter,'yes')
            if strcmpi(tag,'synth')
                ...
                R=lpfilt(All_WV_Data.(eq_id).([Phase '_Waveforms']).R,dt,hf);
                T=lpfilt(All_WV_Data.(eq_id).([Phase '_Waveforms']).T,dt,hf);
                Z=lpfilt(All_WV_Data.(eq_id).([Phase '_Waveforms']).Z,dt,hf);
            else
                ...
                R	= bpfilt(All_WV_Data.(eq_id).([Phase '_Waveforms']).R,dt,lf,hf,'bp');
                T	= bpfilt(All_WV_Data.(eq_id).([Phase '_Waveforms']).T,dt,lf,hf,'bp');
                Z	= bpfilt(All_WV_Data.(eq_id).([Phase '_Waveforms']).Z,dt,lf,hf,'bp');
            end
        end
        phase_st = All_WV_Data.(eq_id).([Phase '_Waveforms']).Window.phase_window(1)-2.02;
        ...
        if strcmp(Phase,'Ps')
            ...
            Z  = taper(Z,Time,P_post_tpr,dt,phase_st,phase_et); % THough maybe we don't need to -- might mask sediments?
            max_Parent_Amp      = max(abs(Z));
            max_Daughter_Amp    = max(abs(R));
        elseif strcmp(Phase,'Sp')
            ...
            R = taper(R,Time,tpr,dt,Taper(1),Taper(2));
            %             plot(SV,'r');
            %             hold off
            %             drawnow
            
            max_Parent_Amp      = max(abs(R));
            max_Daughter_Amp    = max(abs(Z));
        end
        pad = n2-length(Time);
        switch tag
            case 'synth';
                switch Phase;
                    ...
                    case 'Sp'; P = [zeros(1,pad) P];  SV = [zeros(1,pad) SV]; Z = [zeros(1,pad) Z];  R = [zeros(1,pad) R];
                    case 'Ps'; P = [P zeros(1,pad)];  SV = [SV zeros(1,pad)]; Z = [Z zeros(1,pad)];  R = [R zeros(1,pad)];
                end
            case 'data';    P = [P zeros(1,pad)];  SV = [SV zeros(1,pad)]; Z = [Z zeros(1,pad)];  R = [R zeros(1,pad)];
        end
                ...
                if BAZ>=Bin_Data.BAZ_Bins(iBb).BAZ_Range(1) && ...
                    ...
                    for iDb=1:length(Bin_Data.BAZ_Bins(iBb).DIST_Bins);
                        if DIST>=Bin_Data.BAZ_Bins(iBb).DIST_Bins(iDb).DIST_Range(1) && ...
                            ...
                            Z_norm	= Z/max_Parent_Amp;
                            R_norm	= R/max_Parent_Amp;
                            ...
                            if strcmp(Params.Weight_By{1}(1),Phase(1))
                                ...
                                if strcmp(Phase,'Ps')
                                    ...
                                    S2N	= 1/std(R_norm(1:450));
                                elseif strcmp(Phase,'Sp')
                                    % S2N	= 1/std(P_norm(1:450));
                                    S2N	= 1/std(Z_norm(1:450));
                                end
                                if S2N>40
                                    S2N	= 40;
                                end
                            ...
                            if iRFwv==0 || sum(strcmp(fieldnames(RF_Waveforms),eq_id))==0
                                ...
                                RF_Waveforms.(eq_id).Z          = Z_norm*S2N;
                                RF_Waveforms.(eq_id).R          = R_norm*S2N;
...
if ~isempty(All_Event_IDs)
    for ie=1:length(All_Event_IDs)
        ...
        if strcmp(Phase,'Ps')
            P(ie,:)	= WV_Data.(eq_id).Z; %P(ie,:)	= WV_Data.(eq_id).P;    % Parent
            D(ie,:)	= WV_Data.(eq_id).R;%D(ie,:)	= WV_Data.(eq_id).SV;   % Daughter
        elseif strcmp(Phase,'Sp')
            P(ie,:)	= WV_Data.(eq_id).R;   % Parent
            D(ie,:)	= WV_Data.(eq_id).Z;   	% Daughter
        end
    end
            ...
            if(isempty(find(isnan(P_T), 1)) && isempty(find(isnan(D_T), 1)))
                ...
                if strcmp(Phase,'Ps')
                    ...
                    if strcmp(tagd,'HK')
                        ...
                        %[RF_Time,~] = IDRF(Phase,P_T,D_T,dt,t1,t1,1,0.0025,0.0012,200); % 1.5x longer
                    end
                end
            else
...
for in = 1:length(NETs)
    ...
    for is=1:length(STAs)
        ...
        if exist([Proj_Dir NETs{in} '/' STAs{is} '/Ray_Path_Data_',num2str(round(1/Params.hf)),'_',num2str(round(1/Params.lf)),'.mat'],'file')
            load([Proj_Dir NETs{in} '/' STAs{is} '/Ray_Path_Data_',num2str(round(1/Params.hf)),'_',num2str(round(1/Params.lf)),'.mat']);
