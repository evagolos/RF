function CCP_stack_slopeangle(Project, basedir, Phases, tag, varargin)
...
if strcmp(Project(1:4),'Synt'); data_tag='synth'; version=varargin{1};
...
if exist([dir0 version '_2_100.mat'],'file'); load([dir0 version '_2_100.mat']);
    % Append _4_100 to fit file name. -EG 10/2020. Need to change for other period bands.
else disp('No PreCalc RFs name saved!'); return;
end
% PreCalcRFs are sorted by phase, add to name.
pphase= Phases{1};
name=FileNames.(pphase).PreCalcRFs;
...
for iph=1:length(Phases)
    ...
    baz=station_at_cp; RP=station_at_cp; slats=station_at_cp; slons=station_at_cp;
    s_inds=station_at_cp;
    ...
    for jkjk = 1:length(folder)
        ...
        clear dist RFs mask_i Mask_Depth Relative_DT Mask_Time dt T Z
...
%             
%             Sca_info(jkjk).(phase).raypara = RAYPARA;
%             Sca_info(jkjk).(phase).depth = DEPTH;
%             Sca_info(jkjk).(phase).dist = DIST;
%             Sca_info(jkjk).(phase).inc_ang = CON_ANG;
%             Sca_info(jkjk).(phase).gs = GS;
...
%                             cant_use = isinf(t);
    ...
    name=strcat(project, name(1:end-4), '_', num2str(maxdep),'km_', num2str(strike), 'deg.mat');
...
%         ' phase Velocity_Model s_inds slons slats folder station_at_cp time_difference name project Sca_info']) %depths  RP
    FileNames.CCP=name; save([dir0 version '_2_100.mat'],'FileNames');
    save([basedir '/Data/Projects/',project,'/CCP/AllNtwrks_' phase '_' name],'RF_depth','CP_lat','CP_lon','RP','baz','SLAT','SLON','cp_depths','depth_indx',...
        'phase','Velocity_Model','s_inds','folder','station_at_cp','time_difference','name','project','Inter_Velo','numtot','-v7.3')
...
load([Proj_Dir version '_2_100.mat']);
...
%     clear Sca_info
 ...
 %   [latTopo,lonTopo,Topo] = satbath(1,LatLims,LonLims_Temp);
  ...
  %  cd(basedir)
    ...
    %     parfor kkk=1:rfi
    for kkk=1
        parcalculate(kkk,parnum,Proj_Dir,FileNames,RF_depth,tag,s_inds,RP,baz,Inter_Velo,model_lats,model_lons,CP_lat,CP_lon,cp_depths,time_difference,depth_indx,Phase,phase,tmplat,tmplon,latTopo,lonTopo,Topo,SLON,SLAT)
...
function parcalculate(kkk,parnum,Proj_Dir,FileNames,RF_depth,tag,s_inds,RP,baz,Inter_Velo,model_lats,model_lons,CP_lat,CP_lon,cp_depths,time_difference,depth_indx,Phase,phase,tmplat,tmplon,latTopo,lonTopo,Topo,SLON,SLAT)


D=dir([Proj_Dir '/CCP/' FileNames.stack '*']);
inds=zeros(length(D),1);
for k=1:length(D)
    junk=strsplit(D(k).name,'_');
    if strcmp(junk{end}(end-2:end),'mat')
...
%num = 1;
savename=[Proj_Dir '/CCP/' FileNames.stack num2str(num) '.mat'];
...
%     Gs2 = single(1./sqrt(4*pi*Dis_sta.^2));
    ...
    for in = 1:ceil(length(rf_ind)/n_rf)
        ...
        RP_sec = RP(sec_ind);
        [DEPse,RPse] = ndgrid(cp_depths,RP_sec);
        Vinse = repmat(v_inc',[1,length(RP_sec)]);
...
%         SlopeT = abs(atand((Vinc.*sind(Sca).*sind(Mr))./(Vinc.*cosd(Sca)-Vcon.*cosd(Inc))));
        ...
        % New:
        Slope = abs(atand(sqrt((Vinc.^2).*((sind(Sca)).^2)+(Vcon.^2).*((sind(Inc)).^2)-...
        2*Vinc.*Vcon.*sind(Inc).*cosd(Mr).*sind(Sca))./(Vcon.*cosd(Inc)-Vinc.*cosd(Sca))));
        SlopeR = abs(atand((Vcon.*sind(Inc)-Vinc.*sind(Sca).*cosd(Mr))./(Vinc.*cosd(Sca)-Vcon.*cosd(Inc))));
        SlopeT = abs(atand((Vinc.*sind(Sca).*sind(Mr))./(Vinc.*cosd(Sca)-Vcon.*cosd(Inc))));
