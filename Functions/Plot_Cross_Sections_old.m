...
if Ps==1
    ...
    %load([Data,'/Data/Anatolian_Fault_Coordinates.mat']);

%load([Data '/' fol '/nvg_' file]);
%peak=load([Data '/' fol '/envg_' file]);
load([Data '/' fol '/' file]);
...
% load([Data,'/Data/Fault_data/NAsec1.mat']);
% load([Data,'/Data/Fault_data/NAsec2.mat']);
% load([Data,'/Data/Fault_data/NAsec3.mat']);
% load([Data,'/Data/Fault_data/EAsec.mat']);

%load([Data '/Data/Velocity_Models/Mantle/Vs/' VsModelcrName '.mat']);
%load([Data '/Data/Fault_data/seismicity']);
[~,~,Vs_Model.Vs] = gradient(Vs_Model.Vs,1,1,Vs_Model.Depth(2)-Vs_Model.Depth(1));
% 
% 
% load('/Volumes/EARTHSCOPE/CCP_model_Sp_YS_2_33s_300km_9s_arrayMTMfixed_130529.mat');
...
%rplane_ndep=reshape(nvgdeps(:,1),size(tmplat));
%rplane_nbdh=reshape(nvgbreadth(:,1),size(tmplat));
%rplane_pdep=reshape(pvgdeps(:,1),size(tmplat));
%rplane_pbdh=reshape(pvgbreadth(:,1),size(tmplat));
%erplane_ndep=reshape(peak.nvgdeps(:,1),size(tmplat));
%erplane_nbdh=reshape(peak.nvgbreadth(:,1),size(tmplat));
...
direc=direc/ntrans;
switch sects
    case 'NW-SE'; latlons=[54 -87 43.2 -70]; numdeg=0; %numdeg=22; lasc=-1; losc=0;
    case 'SW-NE'; latlons=[29 -87 38.8 -70]; numdeg=0; %lasc=1;  losc=0;
    case 'N-S';   latlons=[46 -87 28 -87];   numdeg=16; lasc=0;  losc=1;
    case 'W-E';   latlons=[33 -120 33 -100]; numdeg=3; lasc=1;  losc=0;
    case 'randomW'; numdeg=0;
        latlons=[40 -122 40 -100];
        %latlons=[-39.91 173.35 -44.39 164.25];
        
    case 'randomN'; numdeg=0;
        %latlons=[40 -105 30 -105];
        latlons=[42 -115 30 -115];
        if iftrans~=0
            for i=1:ntrans%-1
            %for i=ntrans-3
    
                latlons=[latlons;[latlons(1,1)+i*direc(1), latlons(1,2)+i*direc(2),...
                    latlons(1,3)+i*direc(1),latlons(1,4)+i*direc(2)]];
            end
        end
    case 'randomS'; numdeg=0;
        latlons=[31.5 -85.5];%[31.3 -84.5 43.3 -71];
        latlons=[latlons(1) latlons(2) latlons(1)+10.5 latlons(2)+13.5];
    case 'Isabella'    
        numdeg= 0;
        latlons= [36.5 -122.6 36.5 -114.0];
        %latlons= [36.0 -120.0 32.5 -114.0];
        %latlons= [35.8 -121.6 36.8 -116.8];
end
...
% cd([Data '/' fol])
% today = datestr(date, 'dd.mm.yy'); str2=strcat(sects, '_btstrp_orthopack', string, today);
% if ~isdir(str2); eval(['mkdir ' str2]); end
% eval(['cd ' str2])
% fignamestr=[num2str(T) '_' sects '_'];
...
%     load([Data,'/Data/Fault_data/Awatere.mat']);
%     load([Data,'/Data/Fault_data/Wairau.mat']);
     ...
     coplon=[];
     if 0
     for ipf=1:length(NAsec1.lat)-1
         latmin=NAsec1.lat(ipf);
         lonmin=NAsec1.lon(ipf);
         latmax=NAsec1.lat(ipf+1);
         lonmax=NAsec1.lon(ipf+1);
         if lonlima~=lonlimi
            Aco=[(latmax-latmin)/(lonmax-lonmin),-1;(latlons(kj,1)-latlons(kj,3))/(latlons(kj,2)-latlons(kj,4)),-1];
            bco=[-latmin+(latmax-latmin)/(lonmax-lonmin)*lonmin;-latlons(kj,3)+(latlons(kj,1)-latlons(kj,3))/(latlons(kj,2)-latlons(kj,4))*latlons(kj,4)];
            co=Aco\bco;
            if co(1)<lonlimi || co(1)>lonlima || co(2)>latlima || co(2)<latlimi ||...
                    co(1)<min(lonmin,lonmax) || co(1)>max(lonmin,lonmax) || co(2)>max(latmin,latmax)...
                    || co(2)<min(latmin,latmax)
                continue;
            else
                if coplat(1) == 99
                    coplat = [];
            ...
            co(1) = lonlimi;
            co(2) = (latmax-latmin)/(lonmax-lonmin)*(lonlimi-lonmin)+latmin;
            if    co(2)>latlima || co(2)<latlimi || co(2)>max(latmin,latmax) ||...
                    co(2)<min(latmin,latmax) || co(1)<min(lonmin,lonmax) || co(1)>max(lonmin,lonmax)
                continue;
            else
                if coplat(1) == 99
                    coplat = [];
     ...
     for ipf=1:length(NAsec2.lat)-1
         latmin=NAsec2.lat(ipf);
         lonmin=NAsec2.lon(ipf);
         latmax=NAsec2.lat(ipf+1);
         lonmax=NAsec2.lon(ipf+1);
         if lonlima~=lonlimi
            Aco=[(latmax-latmin)/(lonmax-lonmin),-1;(latlons(kj,1)-latlons(kj,3))/(latlons(kj,2)-latlons(kj,4)),-1];
            bco=[-latmin+(latmax-latmin)/(lonmax-lonmin)*lonmin;-latlons(kj,3)+(latlons(kj,1)-latlons(kj,3))/(latlons(kj,2)-latlons(kj,4))*latlons(kj,4)];
            co=Aco\bco;
     ...
     for ipf=1:length(NAsec3.lat)-1
         latmin=NAsec3.lat(ipf);
         lonmin=NAsec3.lon(ipf);
         latmax=NAsec3.lat(ipf+1);
         lonmax=NAsec3.lon(ipf+1);
         if lonlima~=lonlimi
            Aco=[(latmax-latmin)/(lonmax-lonmin),-1;(latlons(kj,1)-latlons(kj,3))/(latlons(kj,2)-latlons(kj,4)),-1];
            bco=[-latmin+(latmax-latmin)/(lonmax-lonmin)*lonmin;-latlons(kj,3)+(latlons(kj,1)-latlons(kj,3))/(latlons(kj,2)-latlons(kj,4))*latlons(kj,4)];
            co=Aco\bco;
...
%     [xs_lo,xs_dp]=meshgrid(lons,deps_tmp);[xs_la,~]=meshgrid(lats,deps_tmp);
%     [ob_x, ob_y, ob_z] = meshgrid(Vs_Model.Longitude,Vs_Model.Latitude,Vs_Model.Depth);
%     obmodel = interp3(ob_x,ob_y,ob_z,Vs_Model.Vs,xs_lo,xs_la,xs_dp,'linear');
%     
    fontsz=20;
    ...
    % Find intersections
    addpath('/Users/evagolos/Research/MIT/Composition/');
...
%     a2=axes('position',[0.03 0.05 0.65 0.4]); hold on
%     contourf(dalj,deps_tmp,obmodel,Npts,'lin nestyle','none'); shading flat;
%     plot(dalj,5*topo_prof,'color',[0 0 0],'linewidth',2);
%     set(a2,'ydir','reverse','ytick',0:50:max_dep,'yticklabel',0:50:max_dep); daspect([1 1 1]);
%     set(a2,'xtick',dalj(1):halfdeg:dalj(end),'xticklabel',dalj2)
%     set(a2,'ygrid','on','xgrid','on','layer','top','fontsize',10,'box','on');
%     axis([lims -25 max_dep]); ylabel('Depth (km)','fontsize',10);
%     title('b) Tomography model (Schmandt et al., 2015 (Schmandt & Lin, 2014))');% Log_1_0(Data Coverage)'); %Wagner Aug. 2014');
%     cmap=flipud(jet); caxis([4.1 4.85]);
%     colorbar('location','southoutside');
%     
% 
%     if ~isempty(Vs_Model.Longitude)
%         [vel_x, vel_y, vel_z] = meshgrid(Vs_Model.Longitude,Vs_Model.Latitude,Vs_Model.Depth);
%         velmodel = interp3(vel_x,vel_y,vel_z,Vs_Model.Vs,x,y,z,'linear');
%         velmodel = reshape(velmodel,length(deps_tmp),length(lons)); 
%     else
%         velmodel = interp1(Vs_Model.Depth,Vs_Model.Vs,deps_tmp');
%         velmodel = repmat(velmodel,1,n);
%     end
%     
